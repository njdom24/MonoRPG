...
namespace RPG
{
    class NPC
    {
        ...
        public NPC(World world, ContentManager content, int[][] colArray, int steps, bool vertical, int posX, int posY, string[] messages, int textWidth = 20, int textHeight = 3)
        {
        ...
        public bool checkPlayer(Player.State state, KeyboardState prevState)
        {
            if (Keyboard.GetState().IsKeyDown(Keys.Space) && !prevState.IsKeyDown(Keys.Space))//doesnt check for OOB!
            {
                if (colArray[y][x + 1] == 2 && state == Player.State.Left)
                {
                    SetState(State.Right);
                    curAnim = walkLeft;
                    return true;
                }
                else if (colArray[y][x - 1] == 2 && state == Player.State.Right)
                {
                    SetState(State.Left);
                    curAnim = walkLeft;
                    return true;
                }
                else if (colArray[y - 1][x] == 2 && state == Player.State.Down)
                {
                    SetState(State.Up);
                    curAnim = walkUp;
                    return true;
                }
                else if (colArray[y + 1][x] == 2 && state == Player.State.Up)
                {
                    SetState(State.Down);
                    curAnim = walkDown;
                    return true;
